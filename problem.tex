...
\added{This project is specifically focused on embedded and networked systems, specifically wireless sensor networks and multi-robotic systems, where timing, communication, interactions with the physical world, and uncertainty are major challenges in modeling the system behaviors.
...
\added{%
  The proposed research is motivated by applications in embedded and networked systems, in particular wirelss sensor networks and multi-robot systems.
  A wireless sensor network (WSN) consists of multiple, often many, sensor nodes that communicate via a wireless network to with servers and/or with other sensor nodes.
  In a WSN, each sensor monitors some physical quantities, such as air temperature, humidity, and CO$_{2}$ concentration level, and exchanges its measurement data with central servers or with other sensors for specific objectives, such as for monitoring and forecasting wildfires.
  Each sensor node typically 
}
