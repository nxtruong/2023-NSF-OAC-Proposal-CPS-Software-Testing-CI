...
\notetruong{I think we should replace ``legacy code'' with a new term to mean code that is written / will be written without any thought / incorporation of testing / verification, i.e., the majority of code in engineering fields.}
...
\deleted{This project is specifically focused on communication protocol implementations in embedded systems where timing is critical to the modeling of behavior, but we expect that our solution will generalize to other critical low-level development efforts because we target the common, hard, case where our approach will be applied to systems with partial or complete existing implementations: typical legacy embedded C code.}
\added{This project is specifically focused on embedded and networked systems, specifically wireless sensor networks and multi-robotic systems, where timing, communication, interactions with the physical world, and uncertainty are some major challenges in modeling the system behaviors.
...
\subsection{\added{Field Application Domains}}
...
\added{%
  ...
  Each sensor node typically has some limited computing power of an embedded microprocessor and limited energy provided by a battery and/or a renewal energy source.
  ...
  In addition, testing and verifying software code of sensor nodes face another significant hurdle due to their nature of interacting directly with the physical world, which involves another level of uncertainty and is not easily replicable in a software testing or verification tool.
  %
  ...
  Due to the complexity of their dynamics and interactions with the physical world, testing and verifying software code of robots, especially cooperative robots in a MRS, are particularly challenging.}
